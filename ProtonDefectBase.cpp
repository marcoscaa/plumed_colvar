...
void ProtonDefectBase::registerKeywords( Keywords& keys ){
...
ProtonDefectBase::ProtonDefectBase(const ActionOptions&ao):
...
ProtonDefectBase::~ProtonDefectBase(){
  delete nl;
}

void ProtonDefectBase::prepare(){
  if(nl->getStride()>0){
...
// calculator
void ProtonDefectBase::calculate()
{
...
//cout << "Tag -1 " << endl;
 ...
 //d is the proton coordination number of the neutral species
 std::vector<double> d;
...
//MCA: Coordination number and charge of species 
 for(unsigned int i=0;i<len_acids;i++) {   
   double sum_tmp = 0.0;
   #pragma omp parallel for reduction(+:sum_tmp)
   for(unsigned int j=len_acids;j<len_acids_hyd;j++) {   
...
// Initialization of the analytical derivatives
#pragma omp parallel for 
...
//Calculation of the analytical derivative of the proton coordination number 
...
//MCA: derivatives for the TotalCharge CV
#pragma omp parallel for
...
//tf = clock();
//total_time = double(tf-t0)/double(CLOCKS_PER_SEC);
//cout << "11 omp_deriv_pos compute: "  << fixed << setprecision(5) << total_time << endl;
//
////cout << "Tag 8 " << endl;
//t0 = clock();
