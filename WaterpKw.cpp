...
class WaterpKw : public WaterpKwBase{
  SwitchingFunction switchingFunction;

public:
  explicit WaterpKw(const ActionOptions&);
// active methods:
...
PLUMED_REGISTER_ACTION(WaterpKw,"SOLVATION")
...
WaterpKw::WaterpKw(const ActionOptions&ao):
Action(ao),
WaterpKwBase(ao) {}
//{
//
...
double WaterpKw::sw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double sw;
...
double WaterpKw::dsw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double d_sw;
 if(abs(arg-sw_shift)/sw_cutoff>(1.-100.0*epsilon) && abs(arg-sw_shift)/sw_cutoff<(1+100.0*epsilon)){
...
//double WaterpKw::pairing(double arg,double&dfunc, double sw_cutoff, double sw_shift, int p, int q){
// sw = (1-pow(((arg-sw_shift)/sw_cutoff),p))/(1-pow(((arg-sw_shift)/sw_cutoff),q));
