...
class SolvationShell : public SolvationShellBase{
  SwitchingFunction switchingFunction;

public:
  explicit SolvationShell(const ActionOptions&);
// active methods:
...
PLUMED_REGISTER_ACTION(SolvationShell,"SOLVATION")

void SolvationShell::registerKeywords( Keywords& keys ){
  SolvationShellBase::registerKeywords(keys);
...
SolvationShell::SolvationShell(const ActionOptions&ao):
Action(ao),
SolvationShellBase(ao) {}
//{
//
//  string sw,errors;
//  parse("SWITCH",sw);
...
double SolvationShell::sw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double sw;
...
double SolvationShell::dsw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double d_sw;
 if(abs(arg-sw_shift)/sw_cutoff>(1.-100.0*epsilon) && abs(arg-sw_shift)/sw_cutoff<(1+100.0*epsilon)){
...
//double SolvationShell::pairing(double arg,double&dfunc, double sw_cutoff, double sw_shift, int p, int q){
// sw = (1-pow(((arg-sw_shift)/sw_cutoff),p))/(1-pow(((arg-sw_shift)/sw_cutoff),q));
