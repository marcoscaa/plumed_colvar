...
class TotalCharge : public TotalChargeBase{
  SwitchingFunction switchingFunction;

public:
  explicit TotalCharge(const ActionOptions&);
// active methods:
...
PLUMED_REGISTER_ACTION(TotalCharge,"TOTALCHARGE")

void TotalCharge::registerKeywords( Keywords& keys ){
  TotalChargeBase::registerKeywords(keys);
...
TotalCharge::TotalCharge(const ActionOptions&ao):
Action(ao),
TotalChargeBase(ao) {}
//{
//
...
double TotalCharge::sw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double sw;
...
double TotalCharge::dsw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double d_sw;
 if(abs(arg-sw_shift)/sw_cutoff>(1.-100.0*epsilon) && abs(arg-sw_shift)/sw_cutoff<(1+100.0*epsilon)){
...
//double TotalCharge::pairing(double arg,double&dfunc, double sw_cutoff, double sw_shift, int p, int q){
// sw = (1-pow(((arg-sw_shift)/sw_cutoff),p))/(1-pow(((arg-sw_shift)/sw_cutoff),q));
