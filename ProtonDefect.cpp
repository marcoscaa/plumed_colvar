...
class ProtonDefect : public ProtonDefectBase{
  SwitchingFunction switchingFunction;

public:
  explicit ProtonDefect(const ActionOptions&);
// active methods:
...
PLUMED_REGISTER_ACTION(ProtonDefect,"ProtonDefect")

void ProtonDefect::registerKeywords( Keywords& keys ){
  ProtonDefectBase::registerKeywords(keys);
...
ProtonDefect::ProtonDefect(const ActionOptions&ao):
Action(ao),
ProtonDefectBase(ao) {}
//{
//
...
double ProtonDefect::sw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double sw;
...
double ProtonDefect::dsw_func(double arg, double sw_cutoff, double sw_shift, int p, int q){
 double d_sw;
 if(abs(arg-sw_shift)/sw_cutoff>(1.-100.0*epsilon) && abs(arg-sw_shift)/sw_cutoff<(1+100.0*epsilon)){
